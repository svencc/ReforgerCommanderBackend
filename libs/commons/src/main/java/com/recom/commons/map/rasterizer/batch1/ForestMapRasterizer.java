...
public class ForestMapRasterizer implements MapLayerRasterizer {
    ...
    public void render(@NonNull final MapComposerWorkPackage workPackage) {
        workPackage.getPipelineArtifacts().getArtifacts().entrySet().stream()
                .filter(entry -> entry.getKey().equals(ForestSpatialIndexCreator.class))
                .findFirst()
                .ifPresentOrElse(
                        (entry) -> {
                            final CreatedArtifact artifact = entry.getValue();
                            final SpacialIndex<ForestItem> spatialIndex = artifact.getData();
                            ...
                            final int[] rawStructureMap = rasterizeForestMap(workPackage.getMapComposerConfiguration().getDemDescriptor(), structureCellSizeInMeter, spatialIndex, workPackage.getMapComposerConfiguration().getMapDesignScheme());
