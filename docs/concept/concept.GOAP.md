...
# Introduction
In the dynamic environment of strategy games, artificial intelligence (AI) is a pivotal factor in crafting an immersive and challenging player experience. 

One of the techniques utilized in the development of AI for strategy games is Goal-Oriented Action Planning (GOAP), a planning system that enables AI agents to respond flexibly and intelligently to the ever-changing conditions in the game environment.

GOAP is based on three core components: **goals**, **actions** and **environment state**.
...
Each action has certain prerequisites and produces specific effects in the game environment. 
...
## World-State
The **World-State** is a representation of the current state of the game environment, encompassing all relevant information that the AI agents need to make decisions. 

The environment-state could include information about the position and health status of units, the state of technology research, control over various territories, and other vital data. 

The environment-state serves as the foundation for the AI agents' planning, enabling them to understand and anticipate the impacts of their actions on the game environment.
...
2. **Reactivity**: Agents can perceive changes in their environment (the game environment) and react to them in real-time, adjusting their behavior and strategies based on the current state of the environment.
...
In summary, an agent in the context of GOAP is a virtual entity that can perceive its environment and take actions to achieve its goals based on its understanding of the environment state and the set of available actions. 

It is the central actor in the GOAP system, utilizing the planning system to navigate and interact with the game environment dynamically and intelligently.
...
In the context of video games, sensors might be virtual constructs that allow an AI agent to gather information about the game environment. 

Here are some examples:
1. **Visual Sensors**: These could be used to perceive the environment visually, identifying objects, obstacles, and other entities in the game environment.
...
In video games, actuators might represent the set of possible actions that an AI agent can perform in the game environment. 

Here are some examples:
1. **Movement Actuators**: These would allow the agent to move within the game environment, navigating towards targets or away from threats.
2. **Manipulation Actuators**: These might allow the agent to interact with objects in the game environment, such as picking up items or using tools.
3. **Communication Actuators**: These could allow communication with other agents or entities in the game environment, allowing for coordinated actions or strategies.
...
## Conclusion
Through the adept combination of goals, actions, and understanding of the environment-state, the GOAP system empowers AI agents in strategy games to create adaptive and realistic plans. 
...
# WorldStates
Various **environment states** and **effects** can be used to control the dynamics of the game and the decisions of the AI. 
...
**Effects** in a GOAP strategy game system refer to the effects that certain actions have on the environment states. 

They are the concrete results or changes caused by performing an action in the game environment.
...
Effects are therefore central to the functioning of a GOAP system, as they form the bridge between actions and goals by defining how actions change environment states and thus influence the achievability of goals.
...
These environment states and effects would serve as the basis for the AI's planning and decision-making. 

Actions in a GOAP system would be defined in terms of their impacts on these environment states and effects, and the planner would try to find a sequence of actions that alters the environment states in a way that achieves a specific goal.
...
Once the action is successfully completed, it will have a defined effect on the environment state, which, in this case, is an increase in the agent's wood resource.

# Introducing the GOAP Planner
The GOAP planner is a sophisticated tool in AI programming that enables dynamic decision-making for AI agents based on the current environment state and a set of predefined goals. 
...
- **1. Goal Setting:**
    ...
    - These goals are based on the current environment state and the overarching objectives of the agent in the game context.
- **2. Action Selection:**
  - Based on the defined goals, the planner selects a set of potential actions that can be performed by the agent. 
  - This set includes all actions that are theoretically possible given the current environment state.
- **3. Checking Preconditions:**
  - For each potential action, the planner checks the preconditions associated with that action to determine if it can be executed in the current environment state.
- **4. Plan Formation:**
  ...
  - This is often done using a planning algorithm that considers the effects of each action on the environment state to find the most efficient path to the goal.
- **5. Execution:**
  - With a plan in place, the AI agent then executes the actions one after the other, constantly monitoring the environment state and adjusting the plan as necessary based on the changing conditions in the game environment.
...
- **Real-Time Adaptability:** The combination of GOAP and FSM allows for real-time adaptability. If the environment state changes in a way that makes the current plan unachievable, the GOAP system can generate a new plan, and the FSM can potentially transition the agent to a different state to adapt to the new circumstances.
...
- **Resource Gathering State:** Based on the environment state, the FSM transitions the agent to a resource-gathering state. The GOAP system then generates a plan of actions to gather the necessary resources.
...
| **Actions**         | Specific tasks or operations that agents can perform in the environment. Each action is characterized by preconditions and effects, and is a fundamental step within a plan that influences the game environment in some way. Actions are aware of their validity and the effects they will have on the environment state.                                                                                                                                                                                |
...
| **Effects**         | The outcomes or changes in the environment state that result from the execution of an action. Effects depict how actions influence the environment and are the changes to the state post the execution of an action.                                                                                                                                                                                                                                                                               |
...
| **Plan**            | A sequence of actions devised by the planner to achieve a specific goal, considering the current environment state and the preconditions and effects of individual actions. A plan takes the agent from a starting state to a state that satisfies a goal.                                                                                                                                                                                                                                             |
